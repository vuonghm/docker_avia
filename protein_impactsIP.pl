...
foreach my $database (@databases){
	if ($database=~/\bPTM\b/){
		...
		foreach my $prot_fn (keys %ptm_prot_db){
			...
			open (REG,"$database_directories/$ptm_prot_db{$prot_fn}{'file'}") or die "Cannot open $ptm_prot_db{$prot_fn}{'file'} file\n";
			while (<REG>){
		...
		if (!-e "$database_directories/$opt_v\_$database.txt"){
			$name="$database_directories/$database";#user specified and may not conform to AVIA
		}
		open (REG,"<$name") or die "Cannot open $name file\n";
		my %href=();
		while (<REG>){
			my ($pp,$info)=split("\t",$_);chomp $info;
			$href{$pp}=$info;
