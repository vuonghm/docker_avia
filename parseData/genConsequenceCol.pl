...
use Data::Dumper;
umask(0000);
use vars qw( $opt_f $opt_a $opt_h $opt_H $opt_o $opt_d $opt_g $opt_G $opt_l $opt_k);
...
while (my $line=<FILE>){
	...
	if ($headerFound){
		...
		for (my $i=0;$i<=$#colElements;$i++){
			...
			}elsif ($colElements[$i]=~/(ANNOVAR annot|Rel pos|Gene$|Annot Feat|Comment|^Chr$)/){
				$annot{$1}=$i;
			}
		}
	 ...
	 if (!$opt_a){
	 	...
	 	if ($first){
	 		...
	 		if (join (":",@colElements[$first..($first+4)])=~/:\s+:/){
	 			...
	 			$consequence.="\t" .join (":",@colElements[$first..($first+4)]). "\t".join ("\t",@colElements[$annot{'ANNOVAR annot'}..$annot{'ANNOVAR annot'}+2,$annot{'ANNOVAR annot'}-1,0..$annot{'ANNOVAR annot'}-2]);
	 		}
