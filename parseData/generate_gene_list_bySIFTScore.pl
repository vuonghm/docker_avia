...
if ($opt_t=~/(c|d|x)/i){
	...
	print "opening $outputfilename.genes_label.txt\n";
}
...
if ($opt_l || $opt_g){
	...
	print "Reading annot file $annot_file\n";
...
if ($namer){
	print "looking for $namer at LN".__LINE__."\n";
	if ( -e "circos.input"){
		$type=`grep $namer circos.input|cut -f3`;chomp $type;
	}
	print "$type\n";
	if (!$type && -e "../config.ini"){#if all else fails 20131213
		$type=`grep viz_type_$namer ../config.ini |cut -f2 -d '='`;chomp $type;
		print "found $type...\n";
		$type||="scatter";
	}else{
		$type="scatter";
