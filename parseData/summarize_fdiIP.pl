...
if (!-e "$root_name.STATS"){
	open (STATS,">$root_name.STATS") or die "Cannot open $root_name.STATS\n";
	print "printing to $root_name.STATS\n";
	print STATS "$header\n";
...
print "Done printint to $root_name.STATS\n";
my $continue=1;
if (-e "viz/subtractive.out" ){
	...
	die "$root_name.annot.txt does not exist...exiting\n";
	system ("grep -e 'exonic' -e 'splic' -e 'UTR' -e intron -e '^#' -e 'Variant ID' -i $root_name.annot.txt |grep -ve 'ncRNA_' -i  >> viz/subtractive.out\n");

	if (`grep sift searchTheseDBs.txt | wc -l`>0){
		my $sift=basename(`grep sift searchTheseDBs.txt`);chomp $sift;
		$sift=~s/\.txt//;
		print "Running: perl $bin/generate_genelists_byANNOVAR_fdi.pl -i viz/subtractive.out -t d -h $sift -o sift\n";
		system("perl $bin/generate_genelists_byANNOVAR_fdi.pl -i viz/subtractive.out -t d -h $sift -o sift\n");
	}else{
