...
my $usage=qq(
	...
	[REQUIRED]
		...
		-t <s|b|c|a> ... what to do
			...
			(c) - generates the gene list for variations where there are hits to the first database
			...
			(x) - generates the gene list for variations where there are no hits to the first database, or vice versa
...
if (!-e "$filename" || -z "$filename") {
	die "You must supply a variations list\n$usage";
}
$opt_b ||= 10_000;
my %genelist;
...
for (my $i=0;$i<=$#headers;$i++){#WHATTODO
	if ($headers[$i]=~/sift/i){
		$header{'s'}=$i;$header{'c'}=$i if ($opt_t=~/c/i && $opt_h=~/avsift/i);
	}elsif ($headers[$i]=~/nonb/i){
		$header{'b'}=$i;$header{'c'}=$i if ($opt_t=~/c/i && $opt_h=~/nonb/i);
	}elsif ($headers[$i]=~/^Gene$/i){
		$header{'gene'}=$i;
...
if ($opt_a && ($whattodo=~/s/ || $opt_h=~/(avsift|ljb_mt|ljb_pp)/)){##REINSTATE if using $opt_a below ###CHILICOOKOFF
	$grep_addon= "grep -e splic -e exonic -e intronic | grep -ve ncRNA -i ";
	$ids=join (",",($header{$whattodo}+1),($header{'annot'}+1) ."-".($header{'annot'}+7));
}elsif ($opt_a){
	$ids=join (",",($header{$whattodo}+1),($header{'annot'}+1) ."-".($header{'annot'}+7));
}elsif ($opt_t=~/d/i){
	$ids=($header{$whattodo}+1);
	system ("head -n1 $filename| cut -f1-".($header{'impact'}+2)."> $opt_o.txt;cut -f $ids $filename >1$opt_o.txt;cut -f1-". ($header{'impact'}+2)." $filename >2$opt_o.txt;paste 1$opt_o.txt 2$opt_o.txt | grep -P '^(Probably ){0,1}Damaging' -i | cut -f 2-100>>$opt_o.txt \n");
...
if ($ans=~/y/i){
	print "[in script:$0]Generating parsing file for ($whattodo)\n cut -f $ids $filename |$grep_addon > $outputfilename\n";
...
if ($opt_a){#places all of the same type into bins (all included) ##CHILICOOKOFF
	...
	LINE: while (my $line=<FILE>){
		...
		if ($whattodo=~/[s]/i && !$opt_a){
			if ( $arr[0]<=0.05){#do not change $arr[NUM]; relative to grep
			...
			GENE: foreach my $gene (@gene){
				...
				$genelist{$gene}{$whattodo}++;
				for (my $k=0;$k<=$#id;$k+=2){
					if (($id[$k]=~/quad/i && $whattodo=~/b/i) || ($whattodo=~/x/i && $id[$k] ne '-')) {print "skipping $id[$k]\n" ;next;}
...
GENE: foreach my $gene (keys %genelist){
	...
	if (exists $genelist{$gene}{'s'} && $genelist{$gene}{'s'}){#WHATTODO
		...
		if (exists $genelist{$gene}{$whattodo} && $genelist{$gene}{$whattodo}){
			if ($whattodo eq 'x'){
				...
				foreach my $id (keys %{$annot{$gene}}){
					if ($annot{$gene}{$id}=~/$regex/i){
						my $annot=lc($1);
						print COMPARE join (" ",$mychr,$mystart,$myend,"$gene","annot=$annot")."\n" ;
						if ($annot=~/exonic/){
							print LABELS3 join (" ",$mychr,$mystart,$myend,"$gene",$fill_color)."\n";
...
if ($opt_t=~/[cx]/i){
	...
	system ("grep -ve '^ ' $outputfilename.genes.txt |sort -u> $outputfilename.genes.txt1;mv $outputfilename.genes.txt1 $outputfilename.genes.txt\n");
	system ("grep -ve '^ ' $outputfilename.genes_label.txt |sort -u> $outputfilename.genes_label.txt1;mv $outputfilename.genes_label.txt1 $outputfilename.genes_label.txt\n");
}
#WHATTODO
if ($opt_t=~/k/i){
...
#					my $foo=`grep $name -i $opt_g`;chomp $foo;print "($foo)\n";next LINE;
	...
	exit (0) if ($type=~/text/i);
	_system ("perl $bin/parseGenesByBin.pl -I $outputfilename.genes_label.txt -o $outputfilename\n");
}
exit(0);
sub _godo{
...
sub readGenes{
	...
	if (-e "genelistwithcoords" && !-z "genelistwithcoords"){
		open (FN,"<genelistwithcoords") or die "Cannot open genelistwithcoords for reading\n";
		while (<FN>){
			my @genearr=split(/\s/,$_);chomp $genearr[$#genearr];
			$genecoords{$genearr[3]}=join (" ",@genearr);
		}
		close FN;
	}else{
