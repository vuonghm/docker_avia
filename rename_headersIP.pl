...
my $headerfound=0;
while (<FILE>){
	if ($_=~/^(#|Chr)/i && $_=~/#{0,1}ANNOVAR\sannot/i){
		$headerfound++;
		...
		for (my $i=0;$i<=$#header;$i++){
			...
			print OUT "#$header[$i]\t" and next if ($header[$i]=~/(ANNOVAR annot|Variant ID|^Gene$|Annot Feat|Rel pos|Gene Ori|Query.*|Allele\d+|Comment|^Chr$)/i);
			$header[$i]=~s/\.exact$//;
